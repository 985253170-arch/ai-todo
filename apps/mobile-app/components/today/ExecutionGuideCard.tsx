...
export function ExecutionGuideCard({
  ...
  const guideContent = isProcessing ? (
  ...
  const canExpand = guide !== null && !isProcessing;
  ...
  return (
    <PaperCard
      ...
      className={
        focused
          ? "min-h-0 flex-1 overflow-hidden bg-paper/90"
          : "min-h-0 flex-1 overflow-hidden bg-paper/90"
      }
    >
      <div className="flex h-full min-h-0 flex-col gap-2">
        ...
        {focused ? (
          ...
          <button
            ...
            onClick={onExpand}
          >
            <div className="line-clamp-4 overflow-hidden text-sm leading-6 text-text-secondary">
              {guideContent}
            </div>
            <span className="mt-2 block text-xs font-semibold text-brand-blue">
              展开查看完整建议
            </span>
          </button>
        ) : (
          <div className="min-h-0 flex-1 overflow-hidden text-sm leading-6 text-text-secondary">
            {guideContent}
          </div>
        )}
      </div>
