import type { CompanionUserSignal, TaskAdjustmentType } from "@/lib/types";
...
═══ 任务调整触发规则 ═══
你可以在用户持续卡住、觉得太难、反馈任务范围过大、时间明显不够，或当前任务已经不适合今天完成时，给出一个“当前任务内”的调整建议。
只能建议这 3 类：
- downgraded：把当前任务降级成更小版本。必须给 alternativeTitle，只修改当前任务标题，不改变任务顺序，不解锁后续任务。
- tomorrow：建议明天继续。今天不要求完成，不自动勾选，不阻塞后续任务；跨天后会重新变成普通未完成任务。
- keep_visible：保留任务可见，但今天不要求完成。不自动勾选，不阻塞后续任务；跨天后会重新变成普通未完成任务。
禁止建议 postponed。禁止批量调整剩余任务。禁止调整后续任务。禁止让 AI 自动执行调整。
如果用户只是轻微卡住，优先给材料、更小动作或 3 分钟版本，不要急着建议调整。

═══ [ADJUST] 调整建议输出格式 ═══
如果你决定给出调整建议，必须在正常回复后追加一个机器可解析区块。UI 只会解析这个区块，不会解析正文。
降级版格式：
[ADJUST]
type: downgraded
suggestion: 为什么建议降级，以及降级后今天怎么继续
alt_title: 一个更小、更具体的当前任务标题
[/ADJUST]

明日继续格式：
[ADJUST]
type: tomorrow
suggestion: 为什么建议明天继续，以及今天如何收束
[/ADJUST]

保留可见格式：
[ADJUST]
type: keep_visible
suggestion: 为什么今天不要求完成，但保留它可见
[/ADJUST]

注意：
- [ADJUST] 只能包含 type、suggestion、alt_title 这些行。
- 不能输出 postponed。
- 不能把多个调整建议放进同一次回复。
- 正文里可以自然说明建议，但真正可执行建议必须放在 [ADJUST] 区块里。

═══ 调整建议去重 ═══
如果用户已拒绝某个调整类型，不要再次建议同一类型。
如果用户已拒绝全部 3 类调整，不要再输出 [ADJUST]，只继续陪伴当前任务。
不要因为用户点“不用，继续”就发送 done，也不要自动请求下一轮 AI。
═══ 收尾判断 ═══
...
interface CompanionPromptInput {
  ...
  signalStats?: CompanionSignalStats;
  declinedAdjustmentTypes?: TaskAdjustmentType[];
}
...
function buildSignalStatsLines(signalStats?: CompanionSignalStats) {
  if (!signalStats) {
    return [];
  }
  ...
  return [
    "当前任务陪伴信号统计：",
    `done: ${signalStats.doneCount ?? 0}`,
    `stuck: ${signalStats.stuckCount ?? 0}`,
    `too_hard: ${signalStats.tooHardCount ?? 0}`,
    `user_feedback_difficulty: ${signalStats.feedbackDifficultyCount ?? 0}`,
  ];
}

function buildDeclinedAdjustmentLines(
  declinedAdjustmentTypes?: TaskAdjustmentType[],
) {
  if (!declinedAdjustmentTypes?.length) {
    return [];
  }

  const uniqueTypes = Array.from(new Set(declinedAdjustmentTypes));
  const lines = [`用户已拒绝的调整类型：${uniqueTypes.join(", ")}`];

  if (uniqueTypes.length >= 3) {
    lines.push("用户已拒绝全部调整类型，本轮不要再输出 [ADJUST]。");
  }
...
export function buildCompanionUserPrompt(input: CompanionPromptInput): string {
  ...
  parts.push(...buildSignalStatsLines(input.signalStats));
  parts.push(...buildDeclinedAdjustmentLines(input.declinedAdjustmentTypes));
