...
- "免费开始使用" → 跳转 `/app`（匿名模式，零门槛）
- "登录" → 跳转 `/login`
- 已登录用户访问 `/` → 自动跳转 `/app`（或显示 Dashboard 入口）
- 简洁、移动端优先
...
- 与当前 `/` 页面功能 100% 一致
- 匿名用户可访问（保持当前低门槛体验）
- 匿名用户 Header 显示"登录"引导按钮 → 点击打开 AuthModal（保留弹窗体验）
- 已登录用户正常使用全部功能
- Header 增加"首页"链接（可选，引导回 `/`）
...
### 3.1 这是 V2.2A 最重要的产品决策

当前 AI Todo 的匿名模式是产品的**核心差异化优势**：
- 用户无需注册即可立即使用
- 输入目标 → AI 拆解 → 执行，零摩擦
- 登录是"增强"而非"门槛"

**V2.2A 的匿名模式决策将影响后续所有版本的产品形态。**

### 3.2 方案对比

#### 方案 A：`/app` 允许匿名访问（保持现状）
...
| 维度 | 评价 |
|------|------|
| 用户体验 | ✅ 零门槛，保持 V1/V2 当前体验 |
| 产品定位 | ⚠️ Landing Page 到 App 的转化路径弱——用户可能永远不登录 |
| 实现复杂度 | ✅ 最低，几乎不需要路由守卫 |
| 数据安全 | ⚠️ 匿名数据在清除浏览器数据后丢失 |
...
| 维度 | 评价 |
|------|------|
| 用户体验 | ❌ 破坏当前匿名模式，增加登录门槛 |
| 产品定位 | ✅ 产品结构清晰，数据稳定 |
| 实现复杂度 | ⚠️ 需要路由守卫 + 匿名体验入口重新设计 |
| 风险 | ❌ **高风险**——破坏现有用户习惯，强制登录可能导致用户流失 |

#### 方案 C：`/app` 允许匿名访问 + Header 强引导登录（推荐）
...
/app    → 所有人可访问（匿名 + 已登录）
         + Header 对匿名用户强引导登录
         + Landing Page 主 CTA 进入 /app 体验
```

| 维度 | 评价 |
|------|------|
| 用户体验 | ✅ 保持零门槛，同时温和引导登录 |
| 产品定位 | ✅ Landing Page 提供产品介绍 → /app 体验 → 登录引导形成自然转化漏斗 |
| 实现复杂度 | ✅ 低，只需 client-side 条件渲染 |
| 数据安全 | ⚠️ 匿名数据风险同方案 A（可接受——用户量小） |

### 3.3 推荐方案：方案 C

**推荐理由**：

1. **不破坏当前体验**：匿名用户仍然可以在 `/app` 完整使用所有功能
2. **建立自然转化漏斗**：Landing Page（了解）→ /app（体验）→ Header 登录引导（注册）
3. **低风险**：不需要 middleware、不需要服务端重定向、不需要重新设计匿名体验入口
4. **为未来留空间**：当用户量和登录转化数据积累后，V2.3+ 可以重新评估是否需要收紧匿名策略
5. **符合产品定位**：AI Todo 的核心价值是"低门槛行动教练"，强制登录违背产品初心

**方案 C 的具体实施**：

| 页面 | 匿名用户 | 已登录用户 |
|------|---------|-----------|
| `/` | 看到 Landing Page，CTA "免费开始使用"→ `/app` | 自动跳转 `/app`（或显示"进入工作台"按钮） |
| `/login` | 看到登录页，可使用验证码/密码登录 | 自动跳转 `/app` |
| `/app` | 完整使用所有功能，Header 显示"登录"引导按钮（打开 AuthModal） | 完整使用所有功能，Header 显示邮箱 + 登出 |

### 3.4 关于方案 B 的明确否决

**V2.2A 不会采用方案 B（强制 `/app` 登录）。** 原因：

- 产品当前用户量极小，强制登录不会带来数据质量提升，只会增加流失
- 匿名模式是 V1.0 至今的核心体验，突然移除会破坏用户信任
- 当前 Auth 体系（V2.1B）已经提供了"低门槛 OTP + 密码"的混合体验，登录本身已经足够简单——应该用引导而非强制

**如果未来需要收紧**（V2.3+），可以在积累用户数据后重新评估。届时可以：
- 限制匿名用户的任务数量（如最多 3 次生成后引导登录）
- 限制匿名用户的历史保留时间（如仅保留 7 天）
- 限制匿名用户不使用 AI 复盘
...
// 无需路由守卫——匿名和已登录用户均可访问
// Header 中通过 user 状态判断显示"登录"还是"已登录"
```
...
**关键点**：
- 无路由守卫——匿名和已登录均可访问
- 极简薄壳，所有逻辑在 MainWorkspace 组件中
- `variant="app"` 告诉 Header 渲染完整版（含登录按钮/邮箱/登出/历史）
...
| `/app` | 已登录 → user 恢复 + 云端加载任务；未登录 → device_id 恢复 + localStorage 加载任务 |

**与当前 `/` 刷新的行为完全一致**（只是 URL 变了）。
...
| # | 验收项 | 预期结果 |
|---|--------|---------|
| 9 | Landing Page CTA "免费开始使用" | 点击后跳转 `/app` |
| 10 | Landing Page "已有账号？登录" | 点击后跳转 `/login` |
| 11 | Landing Page Header "登录"按钮 | 点击后跳转 `/login` |
| 12 | Landing Page Header "免费开始使用"按钮 | 点击后跳转 `/app` |
...
| # | 验收项 | 预期结果 |
|---|--------|---------|
| 22 | 匿名用户在 `/app` 输入目标 → 生成任务 | 正常（device_id 模式） |
| 23 | 匿名用户勾选任务 | 正常 |
| 24 | 匿名用户查看统计 | 正常 |
| 25 | 匿名用户查看历史 | 正常 |
| 26 | 匿名用户使用 AI 复盘 | 正常 |
| 27 | 匿名用户点击 Header "登录" → AuthModal | 弹窗正常，登录后任务迁移正常 |
| 28 | 已登录用户在 `/app` 使用全部功能 | 与当前 `/` 体验完全一致 |
...
| 6 | **SetPasswordPrompt 在 `/login` 和 `/app` 两处同时弹出** | **P2** | 用户在 `/login` 登录成功后跳转 `/app`，两处都检测到 `password_set !== true` | `/login` 的 LoginPageContent 在登录成功后立即处理 SetPasswordPrompt（设置或跳过 → 跳转）。`/app` 的 Header 检测 `user` 变化时用户已设置 password 或已跳过。如果 `/login` 中跳过，跳转到 `/app` 后 Header 会再次检测——这是可接受的（温和引导），不视为 bug |
| 7 | **登录页和 AuthModal 双入口导致困惑** | **P2** | 用户在 `/login` 登录后，在 `/app` 中又看到 AuthModal 的登录入口 | 这是设计意图：`/login` 是从 Landing Page 进入的专用登录页；`/app` 的 AuthModal 是工作台内的快捷登录入口。两者互补，不矛盾 |
