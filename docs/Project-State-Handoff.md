...
> 更新日期：2026-07-09（V3.0A Batch 3A 已完成并已 push）
...
### 9.5 下一阶段：V3.0A Batch 3B

下一阶段为 **V3.0A Batch 3B：任务执行界面**。

Batch 3B 必须继续遵守手机 App 前端边界：任务执行界面是任务层能力，不是全局聊天入口；实现前必须先由 ChatGPT 确认 UI Spec 和执行方案，Codex 只能按精确指令写代码。

## 10. V2.6 → V3.0A 版本路线总览

```
V2.5.3 ✅ → V2.6 ✅ → V2.7 ✅ → V3.0A Batch 3A ✅ → V3.0A Batch 3B 🔜
  AI输出      用户→AI    AI自适应     今日首页/任务界面     任务执行界面
  能力升级    反馈通道    任务调整     手机前端闭环入口     任务层陪伴
```
...
V3.0A 前端隔离工程已启动，并已完成到 **Batch 3A：今日首页 + 任务界面**。
...
2. 不能做成聊天软件
3. 不能新增聊天 Tab
4. AI 陪伴是任务层能力，不是全局聊天入口
5. 必须使用 390×844 手机视口验收
6. 不能出现双重 AppShell
7. 不能把任务界面、任务执行界面混在一个页面
8. 实现前必须先由 ChatGPT 确认 UI Spec 和执行方案
9. Codex 只能按精确指令写代码
