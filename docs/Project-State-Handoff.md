...
> 更新日期：2026-07-20
> 当前状态：V3.0D 已正式完成并关闭；V3.1-A Architecture、Auth Flow Lock、Execution Plan 与 Environment Strategy 均已通过 ChatGPT Review、提交并 Push；A0-D 部分完成；A1 尚未授权，尚未开始代码施工。
> V3.1-A 文档策略提交：`0745b0ca9ca62669d909e5629b24c5183b68db5d` — `docs: finalize V3.1-A execution and environment strategy`（已 Push）
...
## 7. 当前阶段：V3.1-A 文档与 A0-D 已同步

V3.1-A 尚未开始代码施工。Architecture、Auth Flow Lock、Execution Plan 与 Environment Strategy 均已通过 ChatGPT Review，并已随各自文档提交 Push；两次文档提交均不包含代码、环境变量、Key、配置、依赖或数据库变化。
...
| V3.1-A Architecture | 已通过 ChatGPT Review，已提交并 Push。 |
| Auth Flow Lock | 已通过 ChatGPT Review，已提交并 Push。 |
| Execution Plan | 已通过 ChatGPT Review，已提交并 Push。 |
| Environment Strategy | 已通过 ChatGPT Review，已提交并 Push。 |
| Architecture 文档提交 | `bd9d27094dea383651292d2cfe494b939e2537fd` — 已 Push。 |
| 文档策略提交 | `0745b0ca9ca62669d909e5629b24c5183b68db5d` — `docs: finalize V3.1-A execution and environment strategy`，已 Push。 |
| 策略提交文件 | `docs/Execution-Plan-V3.1-A-Mobile-Auth.md`、`docs/Environment-Strategy-V3.1-Development-Production.md`。 |
| 策略提交范围 | 不包含代码、环境变量、Key 或其他文件。 |
| A0-D | 部分完成。 |
| A1 | 尚未授权。 |
| A1.5 | 尚未开始。 |
| Codex | 未获得 V3.1-A 写代码授权。 |
| Production | 尚未创建、尚未启用。 |

### 7.2 Environment Strategy 是环境长期事实源

[Environment-Strategy-V3.1-Development-Production.md](Environment-Strategy-V3.1-Development-Production.md) 是清行 Development / Production 环境配置与交接的长期单一事实源。

新会话、Claude Code、Codex 在处理以下任务前必须优先读取该文档：
...
该文档只记录非敏感环境事实；完整 Key、密码、Token、Cookie、数据库密码不得写入 Git、Markdown 或聊天交接内容。

### 7.3 Development / Production 与 Supabase 状态

```text
Development ≠ Production
```

Development 与 Production 必须隔离 Supabase Project、Auth 用户、Database、环境变量、Site URL、Redirect URL、Email Template、测试账号、正式账号、日志、数据、Vercel Preview 与 Vercel Production。Development 数据不得迁移为正式用户数据；Production 不得复用 Development Auth 用户池和数据库。
...
|---|---|
| 名称 | `qingxing-dev` |
| Project Ref | `hrmxidnjgvdyynbrrxfs` |
| Region | `ap-southeast-1` |
| 地区 | 新加坡 |
| 状态 | `ACTIVE_HEALTHY` |
| 角色 | Development |
| 允许用途 | V3.1-A Auth 开发、OTP、注册、密码、Session、local signOut、Recovery、多标签页、多浏览器 Profile、A1.5。 |
| 禁止用途 | Production、正式用户、正式业务数据、中国大陆正式上线、真实个人主账号测试。 |

完整 publishable key 未写入 Git、Markdown 或聊天交接内容。

**旧 Supabase：**

| 项目 | 当前事实 |
|---|---|
| Project Ref | `vcckgcgrzckxdzwjyvdp` |
| 状态 | `INACTIVE` |
| 角色 | Unknown |
| 隔离规则 | 不恢复、不修改、不用于 Development、不用于 Production、不用于 A1.5、不迁移数据、不读取或输出 Key。 |

当前 Development 已创建独立 Supabase，但本地环境变量与 Gateway 尚未完成。Production 尚未创建、尚未启用，不是当前 A1 阻断条件；Production 最早在 V3.1-A Development 验证关闭后准备，真实业务数据的内部 Production 验证在 V3.1-B 完成后进行，正式开放用户为 V3.3。

### 7.4 A0-D 门禁与当前禁止事项

**A0-D 已完成：**

- 独立 Development Supabase 创建；
- Region 锁定为新加坡；
- Development 角色锁定；
- 旧 Unknown 项目隔离；
- Environment Strategy 已提交。

**A0-D 待完成：**

- mobile 本地 Development 环境变量；
- root 本地 Development 环境变量安全处理；
- 本地 Gateway 工具与固定端口；
- Development Redirect URL；
- 测试账号；
- 六位 OTP Email Template；
- 真实 Recovery 验证条件。
...
- 创建或修改 `.env.local`；
- 写入 Key；
- 修改 Supabase 或恢复旧 Supabase；
- 创建 Production；
- 修改 Vercel；
- 创建 A1.5 Probe；
- commit 或 push Handoff；
- 修改 Execution Plan 或 Environment Strategy。

### 7.5 当前下一步

**下一步：V3.1-A / A1 开工前最终只读核验。**

执行者：**Claude Code**。

目标：

1. 锁定 mobile `.env.local` 位置与变量；
2. 安全处理 root `.env.local` 指向旧 Unknown 项目的问题；
3. 锁定本地 Gateway 工具和固定端口；
4. 确认 A1 最终精确文件范围；
5. 确认 Auth 页面是否需要纳入 A1；
6. 区分 `emailDraft` 与 `AuthUser.email`；
7. 核验依赖安装只修改 mobile package 与 lockfile；
8. 输出 `A1 READY FOR CHATGPT AUTHORIZATION` 或 `A1 BLOCKED`。

只有 ChatGPT 审查并授权后，才可让 Codex 进入 A1。

V3.1-A 当前权威文档为 [Architecture-V3.1-A-Mobile-Auth.md](Architecture-V3.1-A-Mobile-Auth.md)、[V3.1-A-Auth-Flow-Lock.md](V3.1-A-Auth-Flow-Lock.md)、[Execution-Plan-V3.1-A-Mobile-Auth.md](Execution-Plan-V3.1-A-Mobile-Auth.md) 与 [Environment-Strategy-V3.1-Development-Production.md](Environment-Strategy-V3.1-Development-Production.md)；路线以 [Roadmap-V3.0C-to-V3.3-Mobile-Production.md](Roadmap-V3.0C-to-V3.3-Mobile-Production.md) 为准。
...
```text
HEAD / origin/main：0745b0ca9ca62669d909e5629b24c5183b68db5d

docs: finalize V3.1-A execution and environment strategy
已 Push 至 origin/main

提交文件：
- docs/Execution-Plan-V3.1-A-Mobile-Auth.md
- docs/Environment-Strategy-V3.1-Development-Production.md

提交范围：不包含代码、环境变量、Key 或其他文件。
```

### V3.1-A Architecture 文档提交
...
ARCHITECTURE_COMMIT 是 V3.1-A 现行 Architecture 文档提交，不是功能代码基线。
```
...
1. `docs/Project-State-Handoff.md` — 本文档，当前状态。
2. `docs/Environment-Strategy-V3.1-Development-Production.md` — 环境、Auth、Supabase、Vercel、部署任务的长期环境事实源。
3. `docs/Architecture-V3.1-A-Mobile-Auth.md` — V3.1-A 权威 Architecture。
4. `docs/V3.1-A-Auth-Flow-Lock.md` — 现行产品流程锁定。
5. `docs/Execution-Plan-V3.1-A-Mobile-Auth.md` — 已通过并提交的 V3.1-A Execution Plan。
6. `docs/Roadmap-V3.0C-to-V3.3-Mobile-Production.md` — 路线与阶段顺序。

仍需遵守 `CLAUDE.md` 项目规则与 `memory/MEMORY.md` 的按需 Memory 读取策略。无需默认加载大量旧阶段文档；需要历史依据时再按需读取 `docs/archive/` 或既有阶段文档。
