...
| # | 决策 | 说明 |
|---|------|------|
| 1 | **方案 B（/app 必须登录）** | `/app` 仅已登录用户可访问。未登录自动跳转 `/login`。Landing Page 和 Header landing variant 的 CTA 统一指向 `/login` |
...
| **三个薄壳路由页面** | `page.tsx`（Landing，~30 行）、`login/page.tsx`（~30 行）、`app/page.tsx`（~25 行，含登录守卫），只做组装或重定向 |
...
| 2 | `src/app/app/page.tsx` | **新增** | ~25 行 | `/app` 路由页面（含登录守卫） |
...
**新文件**：`src/app/app/page.tsx`（约 25 行）

### 6.1 定位

路由页面 + 登录守卫。**未登录用户自动跳转 `/login`。**

### 6.2 关键设计

- 使用 `useAuth` + `useRouter` + `useEffect`（与 `/` 和 `/login` 相同的守卫模式）
- `isLoading` 或 `!user` 时 return null（避免闪烁）
- 已登录时渲染 MainWorkspace
- 不使用 middleware
...
- 守卫逻辑与 `page.tsx`（`/` 路由）和 `login/page.tsx` 一致
...
| CTA 主按钮 | `<Link href="/login">`，渐变样式与现有按钮一致 |
| 次链接 | `<Link href="/login">`，浅色文字链接 |
| 底部说明 | "登录后使用全部功能。" + `UI_TEXT.PRIVACY_NOTICE` |
...
| Variant | 渲染内容 |
|---------|---------|
| `"landing"` | Logo + "登录"按钮（→ `/login`）+ "开始使用"按钮（→ `/login`）。**不渲染 AuthModal / SetPasswordPrompt / 历史按钮** |
...
| 6 | **未登录用户访问 `/app` 被正确重定向** | **P2** | `/app` 守卫未生效导致未登录用户可访问 | `app/page.tsx` 中使用 `useEffect` + `!isLoading && !user` → `router.replace("/login")` |
...
| `/app` 加登录守卫 | `app/page.tsx` 中使用 `useEffect` + `router.replace("/login")` |
...
| # | 验收项 | 预期结果 |
|---|--------|---------|
| 9 | CTA "免费开始使用" → 点击 | 跳转 `/login` |
| 10 | "已有账号？登录" → 点击 | 跳转 `/login` |
| 11 | Header "登录"按钮 → 点击 | 跳转 `/login` |
| 12 | Header "免费开始使用"按钮 → 点击 | 跳转 `/login` |
...
| # | 验收项 | 预期结果 |
|---|--------|---------|
| 22 | 已登录用户输入目标 → 生成任务 | 正常（user_id 模式） |
| 23 | 已登录用户勾选任务 | 正常 |
| 24 | 已登录用户查看统计 | 正常 |
| 25 | 已登录用户查看历史 | 正常 |
| 26 | 已登录用户使用 AI 复盘 | 正常 |
| 27 | 已登录用户登出 → Header 恢复"登录"按钮 | 正常 |
| 28 | 已登录用户使用全部功能 | 与旧体验完全一致 |
...
| 13 | `/app` 必须加登录守卫 | 方案 B——未登录跳转 `/login` |
...
2. 新增 `src/app/app/page.tsx` — `/app` 路由入口，含登录守卫（useAuth + useRouter + useEffect）。未登录跳转 `/login`（§六）
...
14. **`/app` 必须加登录守卫** — 未登录跳转 `/login`（方案 B）
