...
> **状态：** 已通过 ChatGPT 最终 Review；本文件仍不是 Codex 代码施工授权，D1 需单独授权。
>
...
V3.0D 只修正五项手机端前端体验：Welcome 默认字号单屏优先、OTP Mock 的双状态视觉流程、任务总览的后续任务入口、行动清单二级页、任务执行 default 态页面级内容流调整。
...
| D1 | 本轮修正后的 V3.0D Architecture 与 Execution Plan 经 ChatGPT 最终批准、**共同提交并 Push**后产生的新 HEAD | HEAD = origin/main、tracked clean、staged empty；HEAD 必须已含两份最终修正文档 | ChatGPT 的 D1 Codex 指令；本文件不得猜测 SHA |
...
| D3 | 执行页 default 页面级内容流调整与 compact | `components/today/TaskExecutionView.tsx`、`components/today/ExecutionTaskCard.tsx` |
...
- 顶部右侧“登录” button；Welcome 不保留任何顶部登录入口。
- 底部旧的第二登录 CTA；Welcome 只保留一个“开始使用”主动作。
...
- Welcome 唯一主按钮“开始使用”继续调用 `onNavigate("otp-login")`。
- 新用户从验证码登录页的既有注册入口进入 RegisterPage；已有用户同样点击“开始使用”进入登录页。
- Welcome 不新增注册按钮、“了解更多”、顶部登录按钮或第三个 CTA。
- 保持现有暖米白、深蓝、PaperCard 圆角/阴影 token、PrimaryButton，不重做设计 token；不需要在 Welcome 保留 SecondaryButton。
- 顶部只显示“清行”。

### 5.3 锁定的最终内容顺序与滚动策略

```text
main（默认字号紧凑；AuthShell 内容区可安全纵向滚动）
├─ 顶部品牌区
│  └─ 清行
├─ 中间品牌区
│  ├─ 今天，也从一小步开始
...
默认系统字号下采用紧凑 App Welcome 布局：375–430px 宽度优先让“开始使用”无需滚动即可完整可见。不得再使用 `min-h-screen` 或基于 `100vh` 的最小高度。

系统放大字号或可访问性文字导致内容增高时，允许 AuthShell 既有内容区纵向滚动，确保标题、说明、主视觉和唯一按钮均不裁切；不通过不可读小字号强行塞入单屏，也不声称 Welcome 永远禁止滚动。底部操作区保留安全区与触控尺寸，但不把内部 flex/shrink class 或像素高度定义为产品规则。
...
`isCodeInputFocused` 仅服务视觉焦点格；不创建第二个逻辑验证码状态。验证码确认不增加 `isVerifying`、验证 loading、验证 mock delay 或验证 timeout：完整六码只由用户点击后立即调用 `onLoginSuccess()`，见 §6.6。
...
- 清行品牌；
- 页面标题与说明；
- “验证码登录 / 密码登录”登录方式 Tab（密码登录只能从此 Tab 进入）；
- 邮箱输入；
- “发送验证码”；
- footer 注册入口“第一次来？创建我的行动记录”。

footer 不显示第二个“密码登录”入口，也不新增任何文字链接形式的第三个密码登录入口。注册入口继续使用既有导航进入 RegisterPage；PasswordLoginPage、RegisterPage 与 page.tsx 均不在 D1 修改范围内。
...
**验证的唯一行为：** `verificationCode.length !== 6` 时“验证并进入清行”保持 disabled，且不自动提交。完整 6 位后，用户点击按钮时再次确认 `/^\d{6}$/`、清 `formError`，并**立即**调用现有 `onLoginSuccess()`。不增加 `isVerifying`、验证按钮 loading、验证 mock delay、验证 timeout、成功提示、自动提交、真实 code 校验或 service 调用；email-entry → code-entry 的约 250ms 前端视觉过渡仅属于发送验证码流程，绝不复用于验证按钮。
...
每次 D1 开工时先按 §2.1 核对 ChatGPT D1 指令提供的 SHA；该 HEAD 必须包含本轮共同提交并 Push 的两份最终修正文档。核对完成后重新设置本批 `$D_BASE_HEAD`；不得使用文档编写基线或其他批次变量。
...
- [ ] Welcome 顶部仅“清行”，无顶部登录；底部只有唯一主按钮“开始使用”，无第二登录 CTA、注册按钮、“了解更多”或第三入口。
- [ ] “开始使用”保持调用 `onNavigate("otp-login")`；新用户只能从 otp email-entry 的既有注册入口进入 RegisterPage，未修改 WelcomePage/page.tsx 接口。
- [ ] Welcome 无 `min-h-screen`，没有模拟任务大卡/纸飞机主视觉/网页式功能区。
- [ ] Welcome 原生 img URL 精确为 `/icons/icon-512.png`，无 C4 资产变更。
- [ ] 默认系统字号 375×812 下“开始使用”无需滚动完整可见；系统放大字号允许 AuthShell 内容区安全纵向滚动，且标题、说明、主视觉、按钮均不裁切。
- [ ] OtpLoginPage 不 import/call `loginWithOtp` 或其他 service。
- [ ] email-entry 只显示品牌、页面标题/说明、登录方式 Tab、邮箱输入、发送验证码和 footer 注册入口“第一次来？创建我的行动记录”；密码登录只从 Tab 进入，footer 无第二密码登录或第三文字链接入口。code-entry 只显示锁定的品牌、验证码、Mock、演示邮箱、单逻辑 input/六格、验证、倒计时和更换邮箱内容，无 Tab、密码入口、注册入口、邮箱输入或第三入口。
...
- [ ] 验证按钮未满六码保持 disabled、无自动提交；完整六码后仅用户点击才再次检查 `/^\d{6}$/`、清 `formError` 并立即 `onLoginSuccess()`；无 isVerifying/loading/验证 delay/timeout/成功提示/真实校验/service 调用。
- [ ] 诚实 Mock 文案、演示邮箱标签、无真实发送声称；约 250ms 视觉过渡只发生于 email-entry → code-entry。
- [ ] 60 秒倒计时与 email-entry 发送 timeout/interval/Back cleanup 对称。
- [ ] `otp-code-entry / 65` 依赖完整、卸载 unregister；无新的 history listener。

### 7.3 用户验收

375×812、390×844、430×932 下检查：Welcome 默认字号紧凑、主视觉可辨、唯一“开始使用”按钮完整可触；放大系统字号时 AuthShell 内容区可安全纵向滚动且无内容裁切；email-entry 只含品牌、页面标题/说明、登录方式 Tab、邮箱输入、发送验证码及 footer 注册入口“第一次来？创建我的行动记录”，无 footer 第二密码登录入口；email 校验；点击发送后经约 250ms 前端视觉过渡进入只含锁定内容的 code-entry；code-entry 无登录方式 Tab、密码登录入口、注册入口、原邮箱输入或第三入口；数字键盘；点击视觉格聚焦；粘贴/输入过滤；六格显示；未满六码禁用、无自动提交；完整六码后仅点击才再次检查并立即进入 app，无验证 loading/delay/timeout/成功提示或 service 调用；重发倒计时及归零；更换邮箱回填；系统 Back：code-entry → email-entry → welcome；需要密码登录时必须先回 email-entry 再点击密码登录 Tab。Android Chrome 必测，iPhone Safari 可用时测。

### 7.4 D1 失败与回退

D1 任意问题停止在 D1；不得转入 D2。AuthShell.tsx 在 D1 严格只读：OTP 键盘优先使用既有 AuthShell 滚动能力、浏览器原生聚焦滚动，以及 OtpLoginPage 两文件范围内的布局调整。若仍有遮挡且判断必须修改 AuthShell，立即停止 D1，记录设备、视口、键盘状态和复现步骤，只向 ChatGPT 汇报；不得自行修改 AuthShell，必须由 ChatGPT 决定是否建立独立修复批次与新文件范围。只有 ChatGPT 重新限定时才修。D1 经批准的回退范围仅为 WelcomePage 和 OtpLoginPage 的 D1 commit；不跨批回退。
...
- 第一行：`后面还有 ${remainingCount} 步`，深蓝 serif 或与现有标题同层级；
- 第二行 / 尾部文字箭头：`看看今天的其他小步 →`；
- 不保留旧长段解释或旧入口箭头文案。
...
TaskListView 必须使用**页面内容区单一纵向滚动**：header、TaskProgressCard、hint、CurrentTaskCard、入口卡和 allCompleted 提示均按自然顺序处于同一滚动流。移除 `flex-1` 的 UpcomingTaskList 后，不新增独立小滚动框，不把页面锁死为 `overflow-hidden`。

默认 390×844 优先让当前任务处于视觉前方；375×812 或系统放大字号导致内容超出时，允许页面内容自然纵向滚动。入口卡必须保持完整可读，CurrentTaskCard 不得被压缩或裁切。内容底部预留 BottomTabBar 实际高度 + safe area + 至少 16px，确保末项可达且不被遮挡。维持 currentTask 缺失时 CurrentTaskCard 不渲染、allCompleted 与 hint 的既有条件内容。
...
### 10.3 页面、对外名称与单一滚动内容流

```text
ActionListView
└─ 页面内容滚动区（唯一 `overflow-y-auto overscroll-y-contain`）
   ├─ header
...
内部代码名继续使用 `ActionListView` / `todayMode="action-list"`；“行动清单二级页”只可作为内部文档角色名称，真实 UI 页面标题不得显示“行动清单”。header 返回 button 调 `onBack`，使用现有 `IconBack size={18}` 和至少 `min-h-touch`。标题精确为“今天的其他小步”。说明严格为两行：
...
页面内容滚动区是本页**唯一**纵向滚动区域：header、TaskProgressCard 与全部任务分组处于同一自然内容流。不得将任务 list 单独设为 `min-h-0 flex-1 overflow-y-auto`，不得固定 header/进度后再嵌套列表滚动。内容底部保留 BottomTabBar 实际高度 + safe area + 至少 16px（可使用现有 reserve 变量或等价局部 padding），确保最末卡可达且不被 fixed BottomTabBar 遮挡。外层不能渲染第二 AppShell 或第二 BottomTabBar。

每个分组是语义 section，标题清楚可读。当前任务分组用 `ActionListTaskCard variant="yellow"`；后续分组都用 `variant="white"`。每个分组只渲染分组标题和其锁定的 ActionListTaskCard / 卡片列表；不增加分组尾注文案、空状态说明、基于 currentTask 的临时文案或额外分支文案逻辑。
...
- [ ] ActionListView UI 标题精确为“今天的其他小步”；说明严格为“`不用一次看完，` / `先把眼前这一小步做好。`”；入口卡只显示“后面还有 N 步 / 看看今天的其他小步 →”，无旧长解释或“查看行动清单”。
- [ ] 当前黄色、后续暖白；卡片没有 click/checkbox/任务操作或 Todo 化元素。
- [ ] TaskListView 与 ActionListView 均为页面内容区单一纵向滚动：ActionListView 的 header、进度与任务同一滚动流，无中间独立 list `overflow-y-auto`、无嵌套滚动；末项有 BottomTabBar 实际高度 + safe area + 至少 16px 余量。
...
375×812、390×844、430×932 测试 N=0、N=1、N>=2 且分别有/无 currentTask 的六种组合。确认入口隐藏规则、分组/顺序/卡片颜色；入口只显示“后面还有 N 步 / 看看今天的其他小步 →”；ActionListView UI 标题为“今天的其他小步”，标题下说明为两行“不用一次看完，/先把眼前这一小步做好。”；页面内容单一纵向滚动，header、进度、任务同一流，末项不被 Tab 遮挡，无嵌套小滚动框、checkbox 或点击任务。确认 TaskProgressCard 与 TaskListView 使用同一 `todayState.goal`、`todayState.completedCount`、`todayState.totalCount` 语义与显示进度，且 ActionListView 没有第二进度卡或独立进度计算。确认页面内 Back 与 Android 系统 Back 均为 action-list → tasks → home。
...
D3 只调整 default 态为页面级纵向内容流，并为 ExecutionTaskCard 增加 compact 表示；不以 flex-1 高度竞争作为产品目标。C3 的三枚举、草稿规则、聚焦、keyboard 与 BackController 结构不是重做对象。
...
1. `task.title`：始终完整可读，允许根据可用宽度自然换行；不使用 `line-clamp` 或其他裁切必要标题的方式，也不删除标题。
2. 第一条非空 detail：`task.details?.find((detail) => detail.trim())?.trim()`；存在才显示一次。
...
可压缩/隐藏：多条 details、重复的“现在这件事/正在做”标签（最多保留一个组合提示）、底部完成权长提示、额外装饰、过大留白。卡仍使用 `PaperCard variant="yellow" padding="compact"`，但不写死固定高度，不以 line clamp 或裁切必要内容换取高度。紧凑模式的验收标准是必要信息清楚可读，而不是达到某个像素区间。

## 15. D3-B：TaskExecutionView.tsx default 页面级内容流与三状态

### 15.1 唯一使用位置与 default 内容流
...
在 default 态，header、compact TaskCard、Guide、Feedback 和完成动作必须处于**同一页面级纵向滚动内容流**。内容较长或系统字号放大时，页面自然滚动承载全部内容；不以 `overflow-hidden` 固定高度裁切，不为默认 Guide 创建狭小内部滚动框，也不要求所有内容在任意字号下同时露出。

ExecutionGuideCard 保持独立 AI 指引卡与既有“展开查看完整建议”入口；默认状态只要求内容清晰可识别，不硬编码 4–6 行、不通过极小字号或 line clamp 强行适配。不可改 ExecutionGuideCard 文件。

ExecutionFeedbackBox 保持独立输入区域、现有受控 draft、300 字限制与提交规则；不把源码当前高度定义为产品固定高度，不创建第二输入框，不改 ExecutionFeedbackBox 文件。完成按钮必须可通过页面滚动到达。默认页面内容底部预留 BottomTabBar 实际高度 + safe area + 至少 16px，避免遮挡。
...
| Guide 视觉区域 | 显示、独立 AI 卡，由页面级滚动承载长内容 | 显示、focused 内滚 | 视觉隐藏 |
| ExecutionFeedbackBox 实例 / textarea | 挂载 | 挂载 | 挂载 |
| Feedback 视觉区域 | 显示、独立输入区域 | 视觉隐藏 | 显示、`flex-1` |
| “写下现在的情况” | 隐藏 | 显示 | 隐藏 |
| 完成按钮 | 显示 | 显示 | 显示 |
...
- [ ] compact 始终完整显示 title，允许按可用宽度自然换行；仅显示第一非空 detail（如有）与单次 estimatedMinutes（如有），无占位/编造/重复，且不以固定高度、裁切、line clamp 或极小字号换取紧凑。
- [ ] TaskExecutionView default 唯一传入 `compact`，TaskListView 不使用 ExecutionTaskCard.
- [ ] default 的摘要、AI、输入与完成动作同一页面级纵向滚动流；默认 Guide 无狭小内部滚动框，完成动作可滚动到达，底部预留 BottomTabBar 实际高度 + safe area + 至少 16px。
- [ ] 三枚举状态矩阵、单实例 DOM、draft 生命周期无回归。
- [ ] visualViewport 函数和 two Back handler 的 id/priority/cleanup 零变更。

### 16.3 用户验收

在 375×812、390×844、430×932 的默认字号下验证：compact 卡、AI 输出区和用户输入区清楚可识别，完成按钮可通过页面滚动到达；无 detail/无 estimated/空字符串 detail 的数据均符合缺失规则。系统放大字号下验证：页面自然纵向滚动、无内容裁切、无横向滚动、默认 Guide 无嵌套小滚动框、BottomTab 不遮挡。guide-focused 内部阅读可滚；feedback-focused 键盘弹出、收起后无巨大空白且草稿不丢；Back 链依次为 guide-focused → default → tasks → home，feedback-focused → default → tasks → home。
...
- 是否有第二 AppShell、第二 BottomTabBar、新路由或页面滚动套狭小列表滚动。
- Welcome 是否仅保留“开始使用”，默认字号无需滚动可见、放大字号可安全滚动；是否误恢复登录/注册/第三入口。
- TaskListView、ActionListView、TaskExecutionView default 是否各自采用页面内容区单一纵向滚动；ActionListView 的 header、进度与任务是否同一流，TaskExecutionView default 是否将摘要、AI、输入、完成置于同一流。
- 是否把 compact、AI 或 FeedbackBox 的实现细节误写成固定高度/固定行数，是否裁切必要标题/内容、使用 line clamp 或极小字号强行适配。
- D3 default 是否严格为页面级纵向内容流，而非以 flex-1 高度竞争争取 AI 空间；guide-focused / feedback-focused 是否只保留其 C3 既有 focused 行为。
...
- 是否调用真实 Auth、Supabase、SMTP/API/service，或修改 `services/**`/`types/**`；OTP email-entry 是否仅保留 Tab 作为密码入口与 footer 注册入口，验证码确认是否严格为完整六码点击后立即 `onLoginSuccess()`，无 isVerifying/loading/验证 delay/timeout/自动提交。
- ActionListView 分组是否严格只有标题和锁定卡片/卡片列表，无“完成当前小步后再来看”“现在不用着急”、空状态说明、临时文案或分支文案逻辑。
- D1 是否严格只改 WelcomePage / OtpLoginPage；特别检查 AuthShell 未因 OTP 键盘遮挡而被修改，任何该类遮挡是否已按 D1 停止条件报告 ChatGPT。
...
- Welcome 默认字号紧凑，顶部只有清行，底部只有唯一“开始使用”入口，小径与嫩芽为主视觉；系统放大字号时 AuthShell 内容区安全滚动，不裁切内容。
- OTP email-entry 只显示登录方式 Tab、邮箱输入、发送验证码和 footer 注册入口“第一次来？创建我的行动记录”，密码登录只通过 Tab；code-entry 只显示锁定验证码内容，明确无登录方式 Tab、密码入口、注册入口、原邮箱输入或第三入口；email-entry → code-entry 仅有约 250ms 前端视觉过渡，完整六码验证只在点击后立即 `onLoginSuccess()`，无验证 loading/delay/timeout/service。
- ActionListView 保持清行纸张卡而不是普通 Todo；真实 UI 标题为“今天的其他小步”，标题下说明严格为两行“不用一次看完，/先把眼前这一小步做好。”，入口仅为“后面还有 N 步 / 看看今天的其他小步 →”，当前/后续层级和双条件分组正确；各分组仅有标题与锁定卡片/卡片列表，无尾注文案或临时文案；其进度卡与 TaskListView 复用同一 `todayState` 目标/完成数/总数语义。
- TaskListView、ActionListView 与 TaskExecutionView default 均以页面内容区单一纵向滚动承载长内容；ActionListView 的 header、进度、任务同一流；Bottom Tab 不遮挡内容，无双重界面、无嵌套小滚动框。
- default 执行页当前任务摘要、AI 输出区、用户输入区清楚可识别，完成动作可到达；放大字号时自然滚动、无裁切、无横向滚动。
...
每个批次都必须使用 §2.1 规定、由当批 ChatGPT 指令提供的准确施工 SHA，并在核对 `HEAD = origin/main`、tracked clean、staged empty 后重新设置该批 `$D_BASE_HEAD`。D1 的 `$D_BASE_HEAD` 来自本轮两份最终修正文档经 ChatGPT 最终批准、共同提交并 Push 后的 HEAD；D2 来自已 Review/验收/提交并 Push 的 D1 HEAD；D3 来自已 Review/验收/提交并 Push 的 D2 HEAD。不得以文档编写基线代替施工基线，也不得跨批复用变量。
...
Architecture 原提交
→ Execution Plan 原提交
→ 两份 V3.0D 文档最终一致性修正：ChatGPT 最终批准 → Claude Code 共同精确提交并 Push，形成 D1 施工基线
...
| 级别 | 风险 | 阻断/缓解 |
|---|---|---|
| P0 | Welcome 默认字号或放大字号下遮住唯一主按钮 | 默认字号紧凑；放大字号允许 AuthShell 内容区安全滚动；不缩小字体或裁切 |
...
| P0 | D1 从文档编写 SHA 或旧的单文档提交而非两份最终修正文档共同提交后的 HEAD 开工 | 四层基线；当批 ChatGPT 提供 SHA、核对 HEAD=origin/main 后重新设置 D_BASE_HEAD |
| P0 | ActionListView 撑开 AppShell 或形成嵌套滚动 | 单一页面内容滚动区；header、进度、任务同流；不改 AppShell/BottomTabBar |
...
| P1 | compact 编造/重复详情或时间，或以裁切换紧凑 | 第一条非空 detail + 单次 estimated；不写死高度、不裁切必要内容 |
...
| P1 | ActionListView 偏离现有 TaskProgressCard 进度语义或自行增加分组文案 | 精确复用三个既有必填 Props；不改组件、不新增组件、不重新计算；分组只含标题与锁定卡片/卡片列表 |
...
V3.0D 只有在本轮最终修正后的 V3.0D Architecture 与 Execution Plan 经 ChatGPT 最终批准、共同提交并 Push，随后 D1、D2、D3 各自从当批 ChatGPT 提供且已 Push 的正确施工基线开始，完成批准的实现/Review/构建/验收/提交并 Push，且 D4 全量 Review 与真机矩阵通过、tracked 工作区干净、ChatGPT 明确关闭后才算完成。当前本文件完成不代表 Codex 获得施工授权。
